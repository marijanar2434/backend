...
namespace App\VehicleApp\Vehicle\Main\Infrastructure\Delivery\Symfony\ApiPlatform\DataPersister\Profil\Color;
...
use App\VehicleApp\Vehicle\Main\Application\Command\Profil\Color\CreateColorCommand;
use App\VehicleApp\Vehicle\Main\Application\Command\Profil\Color\DestroyColorCommand;
use App\VehicleApp\Vehicle\Main\Application\Command\Profil\Color\UpdateColorCommand;
...
class DataPersister implements ContextAwareDataPersisterInterface
{
    ...
    { 
        ...
        if ($this->isCreateOperation($context)) {
            $this->commandBus->handle(new CreateColorCommand(
                $data->id,
                $data->name
            ));
        }
        ...
        if ($this->isUpdateOperation($context)) {
            $this->commandBus->handle(new UpdateColorCommand(
                $data->id,
                $data->name
            ));
        }
    ...
    {
        $this->commandBus->handle(new DestroyColorCommand(
            $data->id
        ));
    }
