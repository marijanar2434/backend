...
namespace App\VehicleApp\Vehicle\Main\Infrastructure\Delivery\Symfony\ApiPlatform\DataProvider\Type;
...
use App\VehicleApp\Vehicle\Main\Application\Query\Type\TypeCollectionQuery;
...
class DataProvider implements ContextAwareCollectionDataProviderInterface, RestrictedDataProviderInterface, ItemDataProviderInterface
{
    ...
    public function supports(string $resourceClass, ?string $operationName = null, array $context = []): bool
    {
        ...
        return (Type::class === $resourceClass  && ($context['operation_type'] ?? null) !== 'subresource') ||
            (Type::class === $resourceClass && ($context['operation_type'] ?? null) === 'subresource');
    }
    ...
    public function getCollection(string $resourceClass, ?string $operationName = null, array $context = []): iterable
    {
        ...
        return new Paginator(
            Arr::map($queryResult->getResult(), fn ($dto) => $this->fromTypeDto($dto)),
            (int)Arr::get($context, 'filters.page', 1),
            (int)Arr::get($context, 'filters.size', 10),
     ...
     * @inheritDoc
     */
    public function getItem(string $resourceClass, $id, string $operationName = null, array $context = []): ?Type
    {
        $query = new TypeCollectionQuery(
            1,
            1,
        ...
        return $this->fromTypeDto($queryResult->getFirst());
    }
     ...
     * @return Type
     */
    private function fromTypeDto(TypeDto $dto): Type
    {
    
        $type = new Type();
        $type->id = $dto->getId();
        $type->name = $dto->getName();
        $type->brand = $dto->getBrand();
        return $type;
    }
}
